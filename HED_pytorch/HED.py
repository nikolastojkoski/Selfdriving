...
if __name__ == '__main__':
	...
	#Run HED
	numImages = 0
	for image_file in glob(os.path.join(arguments_strIn, '*.png')):
		...
		edgeMap = PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8))
		edgeMap.save(arguments_strOut + str(numImages) + '.png')

		f, axarr = plt.subplots(1, 2)
		axarr[0].imshow(image)
		axarr[1].imshow(edgeMap)
		plt.show()
